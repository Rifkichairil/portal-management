...
export async function GET() {
  const { data, error } = await supabaseAdmin
    .from("settings")
    .select("id, client_id, client_secret, salesforce_enabled")
...
export async function POST(request: Request) {
  ...
  const { client_id, client_secret, salesforce_enabled } = body;
  ...
  if (existing?.id) {
    ...
    result = await supabaseAdmin
      .from("settings")
      .insert({ client_id, client_secret, salesforce_enabled })
      .select("id, client_id, client_secret, salesforce_enabled")
      .single();
  }
